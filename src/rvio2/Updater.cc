...
Updater::Updater(const cv::FileStorage& fsSettings)
{
    ...
    const float nImageNoiseSigmaX = config_compat::ReadFloat(
        fsSettings, {"cam.sigma_px", "Camera.sigma_px"}, 0.002180293f);
    const float nImageNoiseSigmaY = config_compat::ReadFloat(
        fsSettings, {"cam.sigma_py", "Camera.sigma_py"}, 0.002186767f);
    ...
    mnPubRate = config_compat::ReadFloat(
        fsSettings, {"Displayer.nLandmarkPubRate", "rvio.landmark_pub_rate"}, 5.0f);

    scaleLandmark.x = config_compat::ReadFloat(
        fsSettings, {"Displayer.nLandmarkScale", "rvio.landmark_scale"}, 0.03f);
    scaleLandmark.y = scaleLandmark.x;
    scaleLandmark.z = scaleLandmark.x;
