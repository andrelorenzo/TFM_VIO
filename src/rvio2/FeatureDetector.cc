...
{
    mnImageCols = config_compat::ReadInt(fsSettings, {"cam.width", "Camera.width"});
    mnImageRows = config_compat::ReadInt(fsSettings, {"cam.height", "Camera.height"});

    mnMinDistance = config_compat::ReadFloat(fsSettings, {"Tracker.nMinDist", "rvio.min_feature_distance"}, 15.0f);
    mnQualityLevel = config_compat::ReadFloat(fsSettings, {"Tracker.nQualLvl", "rvio.feature_quality"}, 1e-2f);

    mnBlockSizeX = config_compat::ReadInt(fsSettings, {"Tracker.nBlockSizeX", "rvio.block_size_x"}, 150);
    mnBlockSizeY = config_compat::ReadInt(fsSettings, {"Tracker.nBlockSizeY", "rvio.block_size_y"}, 120);
    ...
    int nMaxFeatsPerImage = config_compat::ReadInt(
