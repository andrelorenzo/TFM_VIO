...
{
    int nIterations = config_compat::ReadInt(fsSettings, {"Tracker.nRansacIter", "rvio.ransac_iter"}, 16);
    mnIterations = nIterations<16 ? 16 : nIterations;

    const int bUseSampson = config_compat::ReadBool(fsSettings, {"Tracker.UseSampson", "rvio.use_sampson"}, true) ? 1 : 0;
    mbUseSampson = bUseSampson;

    mnSampsonThrd = config_compat::ReadFloat(fsSettings, {"Tracker.nSampsonErrThrd", "rvio.sampson_err_thrd"}, 1e-6f);
    mnAlgebraicThrd = config_compat::ReadFloat(fsSettings, {"Tracker.nAlgebraicErrThrd", "rvio.algebraic_err_thrd"}, 1e-3f);
}
