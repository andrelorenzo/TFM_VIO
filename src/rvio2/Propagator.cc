...
Propagator::Propagator(const cv::FileStorage& fsSettings)
{
    ...
    mnSmallAngle = config_compat::ReadFloat(fsSettings, {"imu.small_angle", "IMU.nSmallAngle"}, 0.001745329f);

    mvGyroNoiseSigma = config_compat::ReadImuSigmaGVec(fsSettings, 1e-4f);
    mvGyroRandomWalkSigma = config_compat::ReadImuSigmaWgVec(fsSettings, 1e-6f);
    mvAccelNoiseSigma = config_compat::ReadImuSigmaAVec(fsSettings, 2e-3f);
    mvAccelRandomWalkSigma = config_compat::ReadImuSigmaWaVec(fsSettings, 3e-3f);
