...
\subsection{Navegación autónoma en UAVs}
    ...
    En particular, la odometría visual-inercial (VIO) y otros esquemas de fusión permiten reducir la deriva a corto plazo inherente a la estimación 
...
\section{Arquitecturas de control}
    ...
    destino, teniendo en cuenta la información disponible del entorno. Por su parte, el planificador local genera referencias de movimiento 
    factibles a mayor frecuencia, mientras que el controlador transforma dichas referencias en comandos para los actuadores y las ajusta en 
    tiempo real en función de la estimación del estado y de la información sensorial disponible. En la Figura~\ref{fig:control_theory_autnav} se representa una 
    arquitectura básica que resume estas relaciones funcionales.

    \begin{figure}[H]
    	\ffigbox[\FBwidth] {
            \caption[Arquitectura básica de control en navegación autónoma]{Arquitectura básica de control en navegación autónoma. Fuente: elaboración propia}
    ...
    factible que conecte un estado inicial con un objetivo, imponiendo restricciones geométricas y de seguridad. Por otro lado, el control
    ejecuta dicha solución en el sistema físico, transformando referencias en comandos para los actuadores y compensando perturbaciones y la presencia de obstáculos.
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{0.896\textwidth}{|c|c|c|}
            ...
            \hline
            RRT* & Probabilísticamente completo. & Mayor tiempo de cómputo. \\
            \hline
...
\begin{figure}[H]
    \ffigbox[\FBwidth] {
        \caption[Ejemplo de control por Pure Pursuit]{Ejemplo de control mediante Pure Pursuit. Fuente:~\cite{mathworks_pure_pursuit}.}
...
\end{figure}
    ...
    Generalmente, esta arquitectura se implementa mediante el uso de GNSS/GPS en exteriores o mediante LiDAR y/o cámaras 3D en interiores como fuente global, y el uso de
    encoders o IMUs como fuente odométrica. Estas fuentes se combinan de manera estadística con un filtro, como puede ser un filtro de Kalman. En el caso de interés de este 
    trabajo no se dispone de una fuente global equivalente: la odometría visual-inercial monocular proporciona una estimación odométrica local de la pose a partir de
    características de una imagen 2D junto con la medida de una IMU a alta frecuencia para así mantener una estimación continua del estado, aunque con deriva acumulativa
    si no se incorporan referencias absolutas o cierre de bucle.
    \begin{figure}[H]
        \ffigbox[\FBwidth]{
            \caption[Esquema teórico de localización mediante odometría y sensor global]{Esquema teórico de localización mediante odometría y sensor global. Fuente: elaboración propia}
    ...
    optimización. La IMU permite propagar el estado a alta frecuencia entre imágenes consecutivas, mientras que la información visual reduce la
    deriva acumulada por la integración inercial a corto plazo, aunque no elimina la deriva global.
...
\section{Estimadores de profundidad}
    ...
    (escala y/o desplazamiento) cuando se requiere una medida absoluta.
    ...
    en entrenamiento como en inferencia. Depth Anything 3 demuestra un entrenamiento a gran escala y evalúa variantes con distinto número de 
