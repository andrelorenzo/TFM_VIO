...
estudio se analizó la viabilidad de implementar un sistema de evitación de obstaculos visual para drones de bajo 
...
de profundidad monocular y en la definición de una arquitectura software capaz de ejecutarlos de forma 
...
Las pruebas se realizaron sobre secuencias de 400 frames, midiendo el tiempo medio, mínimo y máximo de inferencia, así 
...
\subsection{1. Estimación de profundidad relativa}
...
estos pixeles válidos (\(Dv\)) se calculá el \textit{ratio} de pixeles válidos con el cociente entre \(sum(Dv)/sum(D)\) y 
...
suficientemente centrada como para que el gradiente salga 0, otro ejemplo de desastre es el caso donde existe demasiado obstaculo y por tanto 
la magnitud de vector de movimiento es demasiado baja. Es por ello que en el nuevo algoritmo se introduce la idea de las 
regiones de interés
...
de que el valor del pixel en \(C(ROI_F)\) sea mayor a un umbral de cercanía y 0 en otro caso. A partir de esta 
...
se puede comprobar en la Figuras~\ref{} y \ref{}.
...
\begin{figure}[H]
    ...
    \caption{ROI de guido dividida en 9 sectores verticales, en azul el sector seleccionado.}
    \label{fig:sectors}
\end{figure}
...
las zonas más lejanas. Para ello, cada píxel del sector recibe un peso proporcional a su profundidad normalizada, elevando
el peso a 2 de manera que, los píxeles correspondientes a regiones lejanas tengan más influencia en el cálculo del punto 
...
para devolverlo como \textit{evade\_angle}~\ref{fig:evade_angle}.
...
\begin{figure}[H]
    ...
    \caption{La flecha indica la direción de evitación decidida por el algoritmo.}
...
la salida de estas 3 variables para un camino con 2 obstáculos (el tercer 1 de la bandera booleana corresponde con el 
aterrizaje del drone en el suelo).
