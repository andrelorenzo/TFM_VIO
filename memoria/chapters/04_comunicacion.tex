...
y del acelerómetro se envían mediante un socket UDP junto con sus respectivos timestamps. Las imágenes RGB se encapsulan en buffers de 
GStreamer, desde donde son codificadas en H.264 y publicadas mediante RTSP. Por otro lado, 
...
\begin{figure}[H]
    \ffigbox[\FBwidth] {
        \caption[Arquitectura de funcionamiento de la pasarela.]{Arquitectura de funcionamiento de la pasarela. Fuente: Elaboración propia}
...
El proceso comienza en la inicialización del receptor RTSP y la creación del socket UDP para la recepción de los 
datos inerciales.

Una vez inicializado el sistema, se lanza un hilo de adquisición. Este hilo se ejecuta iterativamente. Las muestras 
...
Es importante destacar que la sincronización entre vídeo e IMU no se realiza a partir del instante de llegada de los datos a la estación de tierra. Este criterio no resultaría válido, ya que el canal visual y el canal inercial recorren rutas de comunicación distintas y pueden sufrir retardos variables. En su lugar, el sistema conserva las marcas temporales de captura generadas por la propia cámara Intel RealSense D435i. En adquisición local, dichas marcas se obtienen directamente tanto de los fotogramas RGB como de las muestras del acelerómetro y del giróscopo. En adquisición remota mediante RTSP, la marca temporal original del fotograma visual se inserta en el flujo de vídeo codificado y se recupera en recepción, mientras que las muestras IMU se transmiten por UDP manteniendo igualmente su timestamp de captura. De este modo, las imágenes y las medidas inerciales quedan referidas a una misma base temporal asociada al propio sensor.

Con estas marcas temporales, cada imagen se asocia al intervalo inercial comprendido entre el fotograma anterior y el fotograma actual. Para ello, el sistema selecciona las muestras del giróscopo incluidas en ese intervalo y utiliza interpolación lineal para reconstruir valores coherentes de acelerómetro y giróscopo en los instantes temporales necesarios. Además, se fuerza que la última muestra inercial del paquete coincida exactamente con la marca temporal del fotograma actual. Así, el paquete entregado al estimador queda compuesto por una imagen RGB, su timestamp de captura y la secuencia IMU sincronizada correspondiente, evitando depender del jitter de red o del orden de llegada de los mensajes.

Cuando se recibe una imagen RGB válida, el algoritmo selecciona e interpola las medidas del acelerómetro y del giróscopo, generando así una secuencia de muestras 
...
\begin{figure}[H]
    \ffigbox[\FBwidth] {
        \caption[Esquema de adquisición de datos.]{Esquema de adquisición de datos. Fuente: Elaboración propia}
        \label{fig:frame_imu}
    }
...
\begin{figure}[H]
    \ffigbox[\FBwidth] {
        \caption[Ejemplo de sincronización de datos.]{Ejemplo de sincronización de datos. Fuente: Elaboración propia}
...
de guiñada alrededor del eje vertical del marco del vehículo.
...
De esta forma, el sistema garantiza únicamente que las consignas enviadas al UAV se mantienen dentro de los límites por componente 
definidos para la plataforma. La factibilidad dinámica completa depende además de la aceleración, el \textit{jerk} y la respuesta del controlador de vuelo. 
